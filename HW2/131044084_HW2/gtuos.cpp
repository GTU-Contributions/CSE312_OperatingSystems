...
void GTUOS::createProcess(CPU8080 & cpu, char *pN, int ppid){
    ...
    //cout << "CPU PC: " << cpu.state->pc << endl;
    //cout << "This Process PC: " << thisProcess->getRegState().pc << endl;
...
int GTUOS::contentSwitch(CPU8080 & cpu, unsigned int & CRPIndex, int & blockedCycles, int & usedCycles, char *blockedPName, char* nextPName){
    ...
    //printf("Original PC state: %04x\n", (*cpu.state).pc);
    //printf("Saved PC state: %04x\n", pTable.at(CRPIndex)->getRegState().pc);
    ...
    cout << "CONTENT SWITCH: " << CRPIndex << " PID: " << pTable.at(CRPIndex)->getPid() << " PPID: " << pTable.at(CRPIndex)->getPPid() << endl;
    //cout << "PC NEXT " << pTable.at(CRPIndex)->getRegState().pc << endl;
    //cout << "CPU PC NEXT " << cpu.state->pc << endl;
...
int GTUOS::fork(CPU8080 & cpu, unsigned int CRPIndex){
    ...
    //cout << "PARENT PC: " << pTable.at(CRPIndex)->getRegState().pc << endl;
    //cout << "CHILD PC: "  << childProcess->getRegState().pc << endl;
